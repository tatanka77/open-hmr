...
<xsl:template match="span">
  ...
  <xsl:variable name="link" select="normalize-space(a[1]/@href)" />

  <xsl:if test="contains($link,'/television/')">
		<item>
			<title><xsl:value-of select="$title" /></title>
  <xsl:choose>
    <xsl:when test="contains($link, 'http://')">
			<link><script>translate_base_url+"app/tve/series2,,<xsl:value-of select="$link" />";</script></link>
    </xsl:when>
    <xsl:otherwise>
			<link><script>translate_base_url+"app/tve/series2,,http://www.rtve.es<xsl:value-of select="$link" />";</script></link>
    </xsl:otherwise>
  </xsl:choose>
